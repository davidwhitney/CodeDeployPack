param($installPath, $toolsPath, $package, $project)
	...
	#Short circuit if CodeDeployPack has already been installed (this could be by convention in newer versions of NuGet)
	$existingImports = @()
	...
	{
		Write-Host "It looks like CodeDeployPack has been installed, skipping Install.ps1"
		exit
	}
    ...
    $target = $msbuild.Xml.AddTarget("EnsureCodeDeployPackImported")
    $target.BeforeTargets = "BeforeBuild"
    $target.Condition = "'`$(CodeDeployPackImported)' == ''"

    # if the targets don't exist at the time the target runs, package restore didn't run
    $errorTask = $target.AddTask("Error")
    $errorTask.Condition = "!Exists('$relativePath') And ('`$(RunCodeDeployPack)' != '' And `$(RunCodeDeployPack))"
    $errorTask.SetParameter("Text", "You are trying to build with CodeDeployPack, but the NuGet targets file that CodeDeployPack depends on is not available on this computer. This is probably because the CodeDeployPack package has not been committed to source control, or NuGet Package Restore is not enabled. Please enable NuGet Package Restore to download them. For more information, see http://go.microsoft.com/fwlink/?LinkID=317567.");
    ...
    $errorTask = $target.AddTask("Error")
    $errorTask.Condition = "Exists('$relativePath') And ('`$(RunCodeDeployPack)' != '' And `$(RunCodeDeployPack))"
    $errorTask.SetParameter("Text", "CodeDeployPack cannot be run because NuGet packages were restored prior to the build running, and the targets file was unavailable when the build started. Please build the project again to include these packages in the build. You may also need to make sure that your build server does not delete packages prior to each build. For more information, see http://go.microsoft.com/fwlink/?LinkID=317568.");
